...
When Anubis decides it needs to send a challenge to your browser, it sends a challenge page. Historically this page was rendered from an HTML template, but it is now served as a static page that kicks off some JavaScript, reads the challenge information out of the page body, and then solves it as fast as possible in order to let users see the website they want to visit.
